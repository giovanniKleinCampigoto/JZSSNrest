...
/*	private static Functions functions;
	...
	public static void main(String args[]) {
		 ...
		 * percentageInfected =
		 * functions.percentage(functions.getInfected().size());
		 * 
		 * array = functions.getInfected();
		 * 
		 * for (Integer integer : array) { lostPoints += array.get(integer); }
		 * 
		 * int avgWater, avgFood, avgMeds, avgAmmo, totalWater = 0, totalFood =
		 * 0, totalMeds = 0, totalAmmo = 0;
		 * 
		 * inventories.addAll(sc.fetchInventoriesFromDB());
		 * 
		 * for (int i = 0; i < inventories.size(); i++) { totalWater +=
		 * inventories.get(i).getWater(); totalFood +=
		 * inventories.get(i).getFood(); totalMeds +=
		 * inventories.get(i).getMeds(); totalAmmo +=
		 * inventories.get(i).getAmmo(); }
		 * 
		 * avgWater = totalWater / inventories.size(); avgFood = totalFood /
		 * inventories.size(); avgMeds = totalMeds / inventories.size(); avgAmmo
		 * = totalAmmo / inventories.size();
		 * 
		 * }
		 */
	//}	
